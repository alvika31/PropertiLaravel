<x-dashboard-layout>
    ...
    <div x-data="{ 'showModal': false }" @keydown.escape="showModal = false">
        ...
        <button type="button" @click="showModal = true"
            class="px-8 rounded-md py-2 bg-green-500 text-white font-medium"><i class="ti ti-building-factory-2"></i>
            Tambah
        ...
        <div class="fixed inset-0 z-30 flex items-center justify-center overflow-auto bg-black bg-opacity-50"
            ...
            <div class="w-1/2 px-6 py-4 mx-auto text-left bg-white rounded shadow-lg" @click.away="showModal = false">
                ...
                <div class="flex items-center justify-between">
                    <h5 class="mr-3 text-black max-w-none font-medium">Form Tambah Tipe Properti</h5>
                ...
                <div class="border-t-2 border-gray-300 mt-4">
                    <form action="{{ route('tipe-properti.store') }}" method="POST" class="mt-7">
                        @csrf
                        <input type="text" name="nama_tipe" class="w-full rounded-md h-10" placeholder="Nama Tipe"
                            id="">
                        @error('nama_tipe')
                            {{ $message }}
                        @enderror <br>
                        <input type="submit" value="Tambah Tipe Properti"
            ...
            @foreach ($model as $key => $value)
                <tr class="border-y-2 border-gray-100">
                    ...
                    <td class="py-3 text-center">
                        ...
                        <div class="">
                            ...
                            <div class="fixed inset-0 z-30 flex items-center justify-center overflow-auto bg-black bg-opacity-50"
                                ...
                                <div class="w-1/2 px-6 py-4 mx-auto text-left bg-white rounded shadow-lg">
                                    ...
                                    <!-- content -->
                                    <div class="border-t-2 border-gray-300 mt-4">
                                        <form action="{{ route('tipe-properti.update', $value->id) }}" method="POST"
                                            class="mt-7">
                                            @csrf
                                            @method('patch')
                                            <input type="hidden" name="id" id="lokasi-id">
                                            <input type="text" name="nama_tipe" id="nama-lokasi"
                                                class="w-full rounded-md h-10" value="">
                                            @error('nama_tipe')
                                                {{ $message }}
                                            @enderror <br>
                                            <input type="submit" value="Edit Tipe Lokasi"
                                                class="bg-purple-500 px-4 h-8 mt-5 rounded-md text-white">
                        ...
                        <form method="post" action="{{ route('tipe-properti.destroy', $value->id) }}">
                            @csrf
                            <input name="_method" type="hidden" value="DELETE">
                            <button type="submit"
                                class="show_confirm bg-red-500 text-white px-4 py-1 font-medium rounded-md"
                                id="delete-lokasi" data-nama="{{ $value->nama_tipe }}"><i class="ti ti-trash"></i>
                                Delete</button>
                        </form>
                    </td>
                </tr>
    ...
    <script>
        $('.show_confirm').click(function(event) {
            ...
            var namaTipe = $(this).attr('data-nama');
            event.preventDefault();
            Swal.fire({
                title: 'Apakah Anda Yakin?',
                text: "Apakah anda yakin ingin menghapus lokasi " + namaTipe + "",
                icon: 'warning',
        ...
        $('body').on('click', '#btn-edit', function() {
            ...
            $.ajax({
                url: "/dashboard/tipe-properti/" + lokasi_id + "/edit",
                type: "GET",
                success: function(response) {
                    console.log(response.model[0]);

                    $('#lokasi-id').val(response.model[0].id);
                    $('#nama-lokasi').val(response.model[0].nama_tipe);
                }
            })
        });
